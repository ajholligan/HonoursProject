...
X = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, input_size], name='X')
X_img = tf.reshape(X, shape=[-1, input_height, input_width, n_input_channel], name='X_img')
Y = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, n_classes], name='Y')
...
'''
deconv_batch_size = tf.identity(tf.shape(input=feature_vectors)[0], name="deconv_batch_size")
...
'''
ae_error = tf.reduce_mean(input_tensor=list_of_norms(X_decoded - X_true), name='ae_error')
class_error = tf.compat.v1.losses.softmax_cross_entropy(onehot_labels=Y, logits=logits)
class_error = tf.identity(class_error, name='class_error')
error_1 = tf.reduce_mean(input_tensor=tf.reduce_min(input_tensor=feature_vector_distances, axis = 1), name='error_1')
error_2 = tf.reduce_mean(input_tensor=tf.reduce_min(input_tensor=prototype_distances, axis = 1), name='error_2')
...
with tf.compat.v1.Session(config=config) as sess:
    ...
    ########################################################################changes
    n_train_batch = 5#ds_train.batch_size // batch_size
    n_valid_batch = 5#ds_train // batch_size
    n_test_batch = 5#ds_train // batch_size
    # Training cycle
    for epoch in range(training_epochs):
        ...
        if epoch % save_step == 0 or epoch == training_epochs - 1:
            ...
            encode_decode = sess.run(X_decoded,
                                     feed_dict={X: ds_train.train.images[:examples_to_show]})

            # Compare original images with their reconstructions
            f, a = plt.subplots(2, examples_to_show, figsize=(examples_to_show, 2))
            for i in range(examples_to_show):
                a[0][i].imshow(ds_train.train.images[i].reshape(input_height, input_width),
                                cmap='gray',
                                interpolation='none')
                a[0][i].axis('off')
