...
def train(opts):
    ...
    if opts.mode == 'train_mnist':
        train_loader, valid_loader = get_isic_loaders(opts.data_dir, opts.bsize, opts.nworkers, opts.sigma, opts.alpha)
        model = CAE(3, 7, 450, opts.n_prototypes, opts.decoder_arch, channels)
    ...
    for epoch in range(opts.start_epoch, opts.epochs):
        ...
        #valid_sample = torch.stack([valid_loader[i][0] for i in random.sample(range(len(valid_loader)), 10)]).to(device)#########
